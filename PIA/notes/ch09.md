...
@[TOC](目录)
pandas 库的 DataFrameGroupBy 对象是一个存储容器，用于将 DataFrame 行分组到存储桶中。它提供了一组方法来聚合和分析集合中的每个独立组。它允许我们提取每个组中特定索引位置的行。它还提供了一种迭代行组的便捷方法。

```python
import pandas as pd
```

## 从头开始创建 DataFrameGroupBy 对象
...
DataFrameGroupBy 对象根据列中的共享值将 DataFrame 行组织到存储桶中。假设我们对水果的平均价格和蔬菜的平均价格感兴趣。如果我们可以将“水果”行和“蔬菜”行隔离到不同的组中，则执行计算会更容易。

让我们首先在 supermarket DataFrame 上调用 `DataFramegroupby` 方法。我们需要向它传递一个列，pandas 将使用该列的值来创建组，该方法返回一个 `DataFrameGroupBy`。`DataFrameGroupBy` 对象与 DataFrame 是独立且不同的：
...
DataFrameGroupBy 对象擅长聚合操作，比如很容易计算每组数值列的均值。
...
DataFrameGroupBy 对象由一个或一组 DataFrame 组成。在底层，pandas 重复了我们对 “Retail” 使用的提取过程，但对 “Sector” 列中的所有 21 个值进行了提取。我们可以通过将 DataFrameGroupBy 对象传递给 Python 的内置 len 函数来计算 `sectors` 中的组数。`sectors` DataFrameGroupBy 对象有 21 个 DataFrame。 这个数字等于 Fortune 的 Sector 列中唯一值的数量，我们可以通过调用 `nunique` 方法来发现：
...
DataFrameGroupBy 对象上的 `size` 方法返回一个 Series，其中包含按字母顺序排列的组及其行数列表：
...
DataFrameGroupBy 对象的 `first` 方法提取每个组的第一行，如果数据本身是排序的，那么可以获取最大值或者最小值：
...
我们可以调用 DataFrameGroupBy 对象上的方法来将聚合操作应用于每个组。
...
通过一次 sum 方法调用，pandas 将计算逻辑应用于 DataFrameGroupBy 对象中的每个嵌套 DataFrame。DataFrameGroupBy 对象支持很多其他的聚合方法。
...
我们可以通过在 DataFrameGroupBy 对象后面的方括号内传递其名称来定位单列。Pandas 返回一个新对象，一个 SeriesGroupBy：
...
我们可以在这里使用 DataFrameGroupBy 对象的 `apply` 方法。该方法需要一个函数作为参数。它为 DataFrameGroupBy 对象中的每个组调用该函数一次。然后它收集函数调用的返回值并在新的 DataFrame 中返回它们。
...
如何需要对组内进行多函数的运算，可以使用 lambda 函数进行操作：
...
我们可以使用多个 DataFrame 列的值创建一个 DataFrameGroupBy 对象，实现按照多列对 DataFrame 进行分组。
