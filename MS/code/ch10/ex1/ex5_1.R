...
server <- function(input, output, session) {
  output$ui <- renderUI({
    tagList(
      ...
      numericInput("obs1", "Label1", value = 100 * (input$valueB / (input$valueA + input$valueB + input$valueC))),
      numericInput("obs1", "Label1", value = 100 * (input$valueC / (input$valueA + input$valueB + input$valueC)))
    )
  })
}
